...
class VRTrajectoryPublisher(Node):
    ...
    def __init__(self):
        ...
        # Lift control mode:
        #   'head'    -> lift tracks headset height (absolute, cannot hold).
        #   'gesture' -> jog with a held gesture and HOLD on release. Both hands
        #                pinch = lift up, both hands squeeze = lift down. This is
        #                distinct from the reactivate combo (one pinch + one
        #                squeeze) and does not move with the head.
        self.declare_parameter('lift_control_mode', 'head')
        self.declare_parameter('lift_jog_velocity', 0.08)  # m/s while held
        self.declare_parameter('lift_jog_min', 0.0)        # lift_joint lower clamp
        self.declare_parameter('lift_jog_max', 0.45)       # lift_joint upper clamp
        self.lift_control_mode = (
            self.get_parameter('lift_control_mode')
            .get_parameter_value().string_value
        )
        self.lift_jog_velocity = (
            self.get_parameter('lift_jog_velocity')
            .get_parameter_value().double_value
        )
        self.lift_jog_min = (
            self.get_parameter('lift_jog_min').get_parameter_value().double_value
        )
        self.lift_jog_max = (
            self.get_parameter('lift_jog_max').get_parameter_value().double_value
        )
        self.declare_parameter('lift_gesture_threshold', 0.3)
        self.lift_gesture_threshold = (
            self.get_parameter('lift_gesture_threshold')
            .get_parameter_value().double_value
        )
        self._lift_jog_dir = 0          # -1 down, 0 hold, +1 up
        self._lift_jog_command = 0.0    # latched lift target (meters)
        self._lift_jog_last_time = None
        self._lift_jog_log_counter = 0
        ...
        if self.enable_lift_publishing:
            if self.lift_control_mode == 'gesture':
                self.get_logger().info(
                    '[LIFT] Gesture jog mode: RIGHT-hand squeeze = lift up, '
                    'LEFT-hand squeeze = lift down, release/both = hold '
                    f'(speed {self.lift_jog_velocity:.3f} m/s, '
                    f'range [{self.lift_jog_min:.2f}, {self.lift_jog_max:.2f}] m).'
                )
            else:
                self.get_logger().info(
                    '[LIFT] Head-height mode: lift tracks headset height.'
                )
    ...
    def publish_relative_pose(
        ...
        # Only couple head-height changes into arm Z when lift publishing is
        # enabled AND lift is driven by head height. In gesture-jog mode the head
        # does not drive the lift, so it must not drag the arm targets either.
        if (
            self.enable_lift_publishing
            and self.lift_control_mode == 'head'
    ...
    def _update_lift_jog(self, left_state, right_state):
        """Set the lift jog direction from a held two-hand gesture.

        Both hands pinch -> jog up (+1); both hands squeeze -> jog down (-1);
        anything else -> hold (0). The mixed pinch+squeeze combo is reserved for
        the reactivate toggle, so it never jogs the lift.
        """
        if self.lift_control_mode != 'gesture':
            self._lift_jog_dir = 0
            return
        if left_state is None or right_state is None:
            self._lift_jog_dir = 0
            return
        # Hand tracking on this headset does NOT report pinch reliably (pinchValue
        # stays near 0), but squeeze (grip/fist) fires cleanly. So the jog uses
        # squeeze on each hand:
        #   RIGHT squeeze only  -> lift UP
        #   LEFT  squeeze only  -> lift DOWN
        #   both / neither      -> hold
        # Using single-hand squeeze never matches the reactivate combo (one
        # pinch + one squeeze), and squeezing BOTH hands (e.g. a bimanual grasp)
        # just holds the lift instead of moving it.
        thr = self.lift_gesture_threshold
        ls = bool(left_state['squeeze']) or left_state.get('squeezeValue', 0.0) > thr
        rs = bool(right_state['squeeze']) or right_state.get('squeezeValue', 0.0) > thr
        if rs and not ls:
            self._lift_jog_dir = 1
        elif ls and not rs:
            self._lift_jog_dir = -1
        else:
            self._lift_jog_dir = 0

        self._lift_jog_log_counter = getattr(self, '_lift_jog_log_counter', 0) + 1
        if self._lift_jog_log_counter % 15 == 0:
            self.get_logger().info(
                f'[LIFT-JOG] L(s={left_state.get("squeezeValue", 0.0):.2f}/'
                f'{int(left_state["squeeze"])}) '
                f'R(s={right_state.get("squeezeValue", 0.0):.2f}/'
                f'{int(right_state["squeeze"])}) '
                f'dir={self._lift_jog_dir} cmd={self._lift_jog_command:.3f}'
            )
    ...
    async def on_hand_move(self, event, session):
        ...
        try:
            ...
            self._update_lift_jog(left_state, right_state)
    ...
    async def on_body_tracking_move(self, event, session):
        ...
        try:
            ...
            if head_matrix is not None:
                ...
                # Lift: gesture-jog (hold on release) or head-height tracking.
                if self.enable_lift_publishing and self.lift_control_mode == 'gesture':
                    now = self.get_clock().now()
                    if self._lift_jog_last_time is None:
                        self._lift_jog_last_time = now
                    dt = (now.nanoseconds - self._lift_jog_last_time.nanoseconds) / 1e9
                    dt = max(0.005, min(0.2, dt))
                    self._lift_jog_last_time = now
                    if self._lift_jog_dir != 0:
                        self._lift_jog_command += (
                            self._lift_jog_dir * self.lift_jog_velocity * dt
                        )
                        self._lift_jog_command = max(
                            self.lift_jog_min,
                            min(self.lift_jog_max, self._lift_jog_command),
                        )
                    ...
                    point.positions = [float(self._lift_jog_command)]
                ...
                elif self.enable_lift_publishing:
                    now = self.get_clock().now()
